...
getTasks = async function () {
  ...
  try {
    ...
    // ここに SQL を記述する
    const sql =
      "SELECT t_task.id,t_task.task_name,t_task.deadline,t_task.category_id,m_category.category_name,t_task.task_status FROM t_task INNER JOIN m_category ON t_task.category_id = m_category.id";
...
getTasks_status = async function (id) {
  ...
  try {
    ...
    //ここにsqlを記述する↓
    const sql =
      "SELECT t_task.id, t_task.category_id, m_category.category_name, t_task.task_name, t_task.deadline, t_task.task_status, t_task.updated_at, t_task.created_at FROM t_task LEFT JOIN m_category ON t_task.category_id = m_category.id WHERE  t_task.task_status = 1  order by deadline";
